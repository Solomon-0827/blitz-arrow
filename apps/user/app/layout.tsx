...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const defaultMetadata = {
    title: {
      default: site?.site_name || `SecureConnect`,
      template: `%s | ${site?.site_name || 'SecureConnect'}`,
    },
    description: site?.site_desc || '',
    keywords: site?.keywords || '',
    icons: {
      icon: [
        { url: '/ba-smol.svg', type: 'image/svg+xml' },
  ...
  return (
    <html suppressHydrationWarning lang={locale} dir={getLangDir(locale)}>
      ...
      <body
        ...
        className={`${fontMono.variable} size-full min-h-[calc(100dvh-env(safe-area-inset-top))] font-mono antialiased`}
      >
